...
### Atenção - Grande base de dados

**CNPJ´s**

Este download será da maior base de dados desta oficina, são 2.4 Gb compactados, nesta oficina 2 momentos requerem um tempo razoável de processamento o download desta base e a sua passagem para o banco de dados. O tempo de download pode variar conforme a velocidade de sua internet e o tempo de processamento conforme as especificações de seu computador . Sendo assim, será disponibilizado um link com o banco em sqlite já com a base de cnpj´s e na ingestão dos dados será apresentado um exemplo de carga desta base.
...
O chunk acima não está ativo, recomenda-se que baixem o banco já tratado e realizem o download e a ingestão destes dados em uma base com um pouco mais de tempo.
...
Os microdados do CAGED impõem algumas limitações. Ao verificar o [dicionario de dados](ftp://ftp.mtps.gov.br/pdet/microdados/NOVO%20CAGED/Movimenta%E7%F5es/Layout%20Novo%20Caged%20Movimenta%E7%E3o.xlsx) o nomes dos atributos foram escritos com acentuação e espaços e os arquivos são compactados em formato *.7z*. Para a primeira situação realizaremos o tratamento dos atributos, quanto ao formato recomendamos , para o andamento desta ofician, que vá na pasta `caged` do projeto e extraia os arquivos :( .
...
Realizamos a leitura de mais de 16 milhões de linhas com 24 colunas e a carga de mais dados pode começar a ficar difícil. Dando continuidade trataremos os nomes da s colunas retirando os espaços, acentuação para trabalhar com estes dados.

```{r Tratando nomes das colunas CAGED, eval= FALSE}
caged_movimentacao %>%
 names() %>% #pega os nomes das colunas
   stri_trans_general("Latin-ASCII") %>% # substitui caracteres latin para ascii
   str_replace_all(c(" " = "_")) -> names(caged_movimentacao) #troca espaço por _
names(caged_movimentacao)
```

Com os nomes padronizados estamos com mais uma base pronta .

```{r Criando o banco de dados, include=FALSE}
#criando o banco, caso ele exista será somente anexado 
...
#esta funcao e necessaria para alterar o tratamento de entrada dos dados das colunas, se não foi feito desta maneira os dados entram em formado que não será
#possível executar as queries
...
Os dataset vão aparecendo, as necessidade vão aumentando de agregar mais variáveis e chega uma hora que não se tem mais memória para o R trabalhar ou carregar mais dados, somente a base de cnpj 
