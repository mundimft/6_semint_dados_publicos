...
* Download do DB Explorer SQL Lite;
...
* 4) As consultas que vamos realizar podem demorar algum tempo, manteremos vocês informados.(?)
...
Conforme apresentado, o arquivo compactado que estamos tratando possui um *.csv* para cada estado e a quantidade de **1170** observações não parece condizer com a quantidade de empresas inscritas em divida no FGTS. Além disso, os caracteres quebrados podem dificultar as manipulações dos dados. A base foi disponiblizada com caracteres latinos, e precisaremos informar para o pacote para conseguir a visualização adequada. 
...
##Instrução para ler os caractereres corretamete:
locale_padrao_pt <- locale("pt", encoding = "latin1")


#Criando um wrapper para ler todos os dados dentro do arquivo para que o VROOM "entenda" de forma correta
...
**LEMBRAR DE EXCLUIR O CAMPO NOME_DEVEDOR E TROCAR PARA DATA.TABLE**
...
Ao se trabalhar com os microdados do CAGED temos alguns percalços. Ao verificar o [dicionario de dados](ftp://ftp.mtps.gov.br/pdet/microdados/NOVO%20CAGED/Movimenta%E7%F5es/Layout%20Novo%20Caged%20Movimenta%E7%E3o.xlsx) os nomes das colunas foram escritos com acentuação e os arquivos estão compactados em formato *.7z*. Para lidar com a primeira situação, realizaremos o tratamento das colunas. Quanto ao formato, para o andamento desta oficina, recomendamos que vá na pasta `caged` do projeto (?) e extraia os arquivos :(
...
Para fins de aprendizado, disponbilizamos o código de como realizar a ingestão de um grande arquivo sem ter que descompactá-lo e passar diretamente para uma base de dados.
...
# ... Nosssa! Com todas estas bases a memória pode encher. E agora, José?
...
#JOAO, COLOCAR O CHUNK MOVIMENTAÇÃO_GAGED DE VOLTA
 
Agora faremos a cosulta na tabela *empresa* com os mesmos CNAES

```{ r Sumario por cnae_empresa, error=TRUE}
